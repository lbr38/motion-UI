<form id="camera-motion-settings-form" camera-id="<?= $id ?>">
    ...
    foreach ($formParams as $section => $sectionParams) : ?>
        <div class="div-generic-blue bck-blue-alt flex column-gap-10 camera-motion-settings-form-section-toggle-btn margin-top-10 margin-bottom-10 pointer" section="<?= $section ?>">
            <img src="/assets/icons/<?= $sectionParams['icon'] ?>.svg" class="icon-np" />
            <h5 class="margin-top-0 margin-bottom-0"><?= $sectionParams['title'] ?></h5>
        </div>

        <div class="details-div camera-motion-settings-form-section hide" section="<?= $section ?>">
            <?php
...
$(document).ready(function(){
    myselect2.convert('select.param-input-value[param-name="netcam_url"]', 'Select or specify a stream URL...', true, false);
    myselect2.convert('select.param-input-value[param-name="netcam_high_url"]', 'Select or specify a stream URL...', true, false);
});
</script>
