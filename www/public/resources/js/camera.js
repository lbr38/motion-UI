 ...
 * @returns 
 */
async function loadCameras(cameraId = null, disconnect = false)
{
    // Disconnect all existing camera connections if requested
    if (disconnect) {
        console.info('Disconnecting all existing camera connections');

        if (window.cameraInstances) {
            for (const [id, cameraInstance] of Object.entries(window.cameraInstances)) {
                console.info('Disconnecting camera #' + id);

                Camera.safeDisconnect(cameraInstance, id);
            }
        }
    }
