...
<body>
                ...
                <div id="alert-div" class="config-div hide">
                    ...
                    <form id="alert-conf-form" autocomplete="off">
                        <table id="motion-alert-table">
                             ...
                             */
                            if (!file_exists('/etc/motion/motionui/event')) {
                                echo '<tr><td colspan="100%"><p class="yellowtext">The bash script <b>/etc/motion/motionui/event</b> is not found. You may configure alerts but you will not receive them.</p></td></tr>';
                            }
    ...
    <div id="how-to-alert-container" class="config-div hide">
        ...
        <p>
            ...
            - Create a new configuration file <b>/etc/motion/motionui/.muttrc</b>. You can create it anywhere else but it should be readable by motion user.<br>
            - Insert your mutt configuration (you can easily find exemples on the Internet) and check if motion can send a mail:<br>
        </p>
        <pre>sudo -u motion echo '' | mutt -s 'test' -F /etc/motion/motionui/.muttrc myemail@mail.com</pre>
        <br><br>

        <p>
            2. Be sure that the <b>event</b> script is present in <b>/etc/motion/motionui/</b>.<br>
        ...
        <pre>sudo chmod 550 /etc/motion/motionui/event<br>sudo chown motion:motionui /etc/motion/motionui/event<br><br>ls -l /etc/motion/motionui/event<br>-r-xr-x--- 1 motion motionui 5667 juil. 24 14:18 /etc/motion/motionui/event</pre>
        <br><br>
        ...
        <pre>Parameter: on_event_start<br>Value: /etc/motion/motionui/event --cam-id %t --register-event %v</pre>

        <p>
            - Enable and edit this parameter to be sure that the event end will be take into account:
        </p>
        <pre>Parameter: on_event_end<br>Value: /etc/motion/motionui/event --end-event %v</pre>
        <p>
        ...
        <pre>Parameter: on_movie_end<br>Value : /etc/motion/motionui/event --event %v --file %f</pre>
        <p>
        ...
        <pre>Parameter: on_picture_save<br>Value: /etc/motion/motionui/event --event %v --file %f</pre>
        <p>
